...
const getFileName = (rootContract: GatheredContractInterface) =>
  path.parse(rootContract.url).base;

const getFilePath = (fileName: string, outputDirectory: string) =>
  path.join(outputDirectory, fileName);

export async function flattenSingleFile(input: InputConfig, name: string) {
  const config = inputToConfig(input);
  const output = await getContractDependency(config);
  const contract = output.find(
    (contracts) => getFileName(contracts[contracts.length - 1]) === name
  );
  if (!contract) {
    return null;
  }
...
async function getContractDependency(
  config: Config
): Promise<GatheredContractInterface[][]> {
  const resolver = ImportsFsEngine().addResolver(
    resolvers.BacktrackFsResolver(config.nodeModulesDirectory)
  );

  const allContracts = findInputs(
    config.sourceDirectory,
    getExtensionForCompilerType(config)
  );

  return Promise.all(
    allContracts.map(async (contract) =>
      gatherSourcesAndCanonizeImports([contract], ".", resolver)
    )
  );
}
...
function getFlattenedSource(
  ...
  const contractsWithCommentedDirectives = contract.map(
    replaceDirectivesWithComments(rootContract)
  );
  const source = "".concat(...unique(contractsWithCommentedDirectives));
  const sourceWithNormalizedLicences = normalizeSpdxLicenceIdentifiers(
    source,
    fileName
  );
...
function saveToFile(
  ...
  output.map((contract: Array<GatheredContractInterface>) => {
    const { filePath, sourceWithNormalizedLicences } = getFlattenedSource(
      contract,
      outputDirectory
    );
...
function replaceDirectivesWithComments(
  rootContract: GatheredContractInterface
) {
  ...
  return (dependency: GatheredContractInterface) => {
    const sourceWithImportsWithRelativeImports = dependency.source.replace(
      IMPORT_NODE_MODULES_REGEX,
      "$1$2"
    );
    const sourceWithCommentedImports =
      sourceWithImportsWithRelativeImports.replace(
        IMPORT_SOLIDITY_REGEX,
        "// $&"
      );
    ...
    const sourceWithCommentedPragmas = sourceWithCommentedImports.replace(
      PRAGMA_SOLIDITY_REGEX,
      "// pragma solidity"
    );
    return (
      `// Dependency file: ${filePath}\n\n` +
...
export function normalizeSpdxLicenceIdentifiers(
  flattenContracts: string,
  contractName: string
) {
  const LICENCE_REGEX = /^\s*\/\/\s*SPDX-License-Identifier:(.*)$/gm;
