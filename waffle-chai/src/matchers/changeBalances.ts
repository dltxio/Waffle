...
export function supportChangeBalances(Assertion: Chai.AssertionStatic) {
  Assertion.addMethod(
    "changeBalances",
    function (this: any, accounts: Account[], balanceChanges: BigNumberish[]) {
      callPromise(this);
      const isNegated = this.__flags.negate === true;
      const derivedPromise = this.callPromise
        .then(() => {
          if (!("txResponse" in this)) {
            throw new Error(
              "The changeBalances matcher must be called on a transaction"
            );
          }
          return Promise.all([
