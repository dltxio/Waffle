...
describe("INTEGRATION: Matchers: revertedWith", () => {
  ...
  describe("Custom errors", () => {
    ...
    it("Revert fail", async () => {
      const matchers = await deploy();
      await expect(
        expect(matchers.doRevertWithOne()).to.be.revertedWith("Two")
    ...
    it("With args and big number success", async () => {
      ...
      await expect(matchers.doRevertWithBigNumber())
        .to.be.revertedWith("One")
        .withArgs(
          BigNumberish.from("9007199254740991000000"),
          "message",
          "0x00cfbbaf7ddb3a1476767101c12a0162e241fbad2a0162e2410cfbbaf7162123"
        );
    });
    ...
    it("With args and big number failure", async () => {
      const matchers = await deploy();
      await expect(
        expect(matchers.doRevertWithBigNumber())
          .to.be.revertedWith("One")
          .withArgs(
        ...
        'Expected "9007199254740991000000" to be equal 9007199254740991000001'
      );
    });
    ...
    it("With args failure", async () => {
      const matchers = await deploy();
      await expect(
        expect(matchers.doRevertWithOne())
          .to.be.revertedWith("One")
          .withArgs(
            1,
            "message",
    ...
    it("With array args failure", async () => {
      const matchers = await deploy();
      await expect(
        expect(matchers.doRevertWithTwo())
          .to.be.revertedWith("Two")
          .withArgs(
            [1, 2, 4],
            [
  ...
  it("Revert success (decorated)", async () => {
    const matchers = await deploy();
    await expect(
      matchers.doRevertWithDecoratedCustomErrorName()
  ...
  it("With args failure (decorated)", async () => {
    const matchers = await deploy();
    await expect(
      expect(matchers.doRevertWithDecoratedCustomErrorName())
        .to.be.revertedWith("$__DecoratedCustomErrorName")
        .withArgs(
          1,
          "message",
