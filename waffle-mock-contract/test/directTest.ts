...
export function mockContractDirectTest(provider: MockProvider) {
  ...
  describe("Mock Contract - Integration (called directly)", () => {
    ...
    it("should be able to call to another contract", async () => {
      const counterFactory = new ContractFactory(
        Counter.abi,
        Counter.bytecode,
        wallet
      );
    ...
    it("should be able to call another contract with many parameters", async () => {
      ...
      expect(
        await mockCounter.staticcall(counter, "addThree", 1, 2, 3)
      ).to.equal("6");
    });
    ...
    it("can be deployed under specified address", async () => {
      ...
      expect(await provider.getCode(address)).to.eq(
    ...
    it("can be overidden", async () => {
      ...
      const counter = await counterFactory.deploy();
      expect(await provider.getCode(counter.address)).to.eq(
        "0x" + Counter.evm.deployedBytecode.object
      );
